    ...
    <main className="flex grow flex-col justify-center space-y-8 px-8">
      <Scoreboard
        ...
        host={{
          ...
          marker: isSpectator ? getAssignedMarker(session, "host") : yourMarker,
      ...
      <Button
        className={cn(
          (session.status === "ongoing" || role === "spectator") && "invisible",
        )}
        disabled={session.status === "ongoing" || role === "spectator"}
        onClick={() => {
